...
public class Player {
    ...
    private void MoveLeft(){
        entity.getBody().applyForce(new Vec2(-6.0f, 0.0f), entity.getBody().getWorldCenter());
    }
    ...
    private void Jump(){
        if(entity.getPosition().y <= Constants.GROUND_LEVEL + Constants.BALL_RADIUS + Constants.EPSILON)
            entity.getBody().applyForce(new Vec2(0.00f, 300.0f), entity.getBody().getWorldCenter());
    }
